...
export var gol_scena_key:String = 'bug_acid'
...
var Bullet = preload("res://src/weapons/bug_acid/acidball.tscn")
...
func _event_handler_shoot(data:Dictionary):
	...
	if data.has('direction_x') and data.has('direction_y') :
		...
		if !isColddown:
			...
			bull.power = power
			bull.init_shoot_direction(global_position+Vector2(0,-10).rotated(global_transform.get_rotation()),Vector2(data['direction_x'],data['direction_y']),bull.id)
...
func _remove_bullet_from_map(id):
	if map_bullet.has(id):
		print('remove bullet from ',name,' id:',id)
